...
def generateNutritionReport():
    ...
    try:
        ...
        with open('product_list.csv', 'w+', newline='') as cUM:
            fUM = ['Source', 'Brand', 'Product name', 'Category', 'Pack size', 'Serving size', 'Servings per pack', 'Energy per 100g (or 100ml)', 'Protein per 100g (or 100ml)', 'Total fat per 100g (or 100ml)', 'Saturated fat per 100g (or 100ml)', 'Carbohydrate per 100g (or 100ml)', 'Sugars per 100g (or 100ml)', 'Sodium per 100g (or 100ml)', 'UID', 'Product code', 'Nutrition info from match', 'Dietary code']
            ...
            for row in tqdm(cur):
                ...
                #check if nutrition data exists for row
                if (row[9]=='' or row[9]==' '):
                    cur2 = conn.cursor(buffered=True)
                    sql2 = 'SELECT UID1, UID2 FROM matches WHERE UID1=\''+row[14]+'\' OR UID2=\'' + row[14]+'\''
                    cur2.execute(sql2)
                    result = cur2.fetchone()
                    if result is not None: 
                        uidToExtract = ''
                        if result[0] == row[14]:
                            uidToExtract = result[1]
                        else:
                            uidToExtract = result[0]
